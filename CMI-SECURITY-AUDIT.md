...
| 4 | Medium (race) | Two concurrent initiates for the same booking crashed the loser on the order’s unique index (P2002 → customer-facing 500). | P2002 is caught; the loser loads and reuses the winner’s order (same oid — exactly the retry semantics). The key is now `Order.bookingRef`, which is never null — a unique index on the optional `reservationId` would have let only one day-pass/transfer order exist. |
